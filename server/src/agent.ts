        ...
        const itemLookupTool = tool(
            async ({query,n = 10}) => {
                try {
                    ...
                    if(result.length === 0){
                        console.log(`Vector search returned no results, trying texxt search`)
            ...
            {
                name: "item_lookup",
                description: "Gathers furniture item details from the inventory_database",
                schema: z.object({
...
Current time: {time}`
                ...
                const result = await model.invoke(formatedPrompt)
        ...
        if(error.status === 429) {
            throw new Error("Service temporarly unavailable due to rate limits.")
        } else if(error.status === 401) {
            throw new Error("Authentication failed. Please check you API configuration")
        } else{
