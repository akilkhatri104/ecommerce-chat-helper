...
async function setupDatabaseAndCollection(): Promise<void> {
    ...
    if (collections.length === 0) {
        ...
        console.log(
            "Created 'items' collection in 'inventory_database' database"
        );
    } else {
        console.log(
...
async function createVectorSearchIndex(): Promise<void> {
    try {
        ...
        const vectorSearchIdx: SearchIndexDescription = {
                name: "vector_index",
                type: "vectorSearch",
                definition: {
                    fields: [
        ...
        console.error("Faild to create vector search index: ",error);
    }
}
...
async function createItemSummary(item:Item) : Promise<string> {
    return new Promise((resolve) => {
        ...
        const price = `At full prie it costs: ${item.prices.full_price} USD, On sale it costs: ${item.prices.sale_price} USD`
...
async function seedDatabase(): Promise<void> {
    try {
        ...
        const recordsWithSummuries = await Promise.all(
        ...
        for(const record of recordsWithSummuries){
        ...
        console.error("Errror seeding database: ",error);
    }finally {
        await client.close()
    }
}
